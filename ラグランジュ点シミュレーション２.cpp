...
int cul();
inline float cul_a(float, float, float, float, float, float);
...
int main()
{
    ...
    for (int i = 0; i < times; i++)
    {
        ...
        for (int j = 0; j < p - 1; j++)
        {
            fprintf(gp, "%d, %d\n", (int)(x[j] * cos(ω * t_1) + y[j] * sin(ω * t_1)), (int)(-x[j] * sin(ω * t_1) + y[j] * cos(ω * t_1)));
        }
        fprintf(gp, "e\n");
...
int cul()
{
    ...
    vx_1 = vx_0 + (cul_a(x_0, y_0, xp_0, yp_0, xq_0, yq_0) + cul_a(x_1, y_1, xp_1, yp_1, xq_1, yq_1)) / 2.0F * dt;
    vy_1 = vy_0 + (cul_a(y_0, x_0, yp_0, xp_0, yq_0, xq_0) + cul_a(y_1, x_1, yp_1, xp_1, yq_1, xq_1)) / 2.0F * dt;
...
inline  float cul_a(float x1, float y1, float x2, float y2, float x3, float y3)
{
